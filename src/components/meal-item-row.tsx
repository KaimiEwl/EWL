      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1 flex flex-wrap gap-1.5 text-[11px] font-medium text-slate-500">
                ...
                <span className="theme-elevated rounded-full px-2 py-1">{currentLabel}</span>
          ...
          {expanded ? (
            <>
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-500">
                ...
                <div className="rounded-[1rem] bg-slate-50 px-3 py-2">{currentLabel}</div>
              </div>

              <div className="theme-elevated mt-3 rounded-[1rem] px-3 py-3 text-xs text-slate-600">
                <div className="font-semibold text-slate-800">Сейчас: {currentLabel} • {row.nutrition.kcal} ккал</div>
                {hasChanges && nextLabel && nextNutrition ? (
                  <div className="mt-1">Будет: {nextLabel} • {nextNutrition.kcal} ккал</div>
                ) : null}
              </div>
          ...
          <button
            ...
            aria-label={expanded ? "Свернуть продукт" : "Развернуть продукт"}
            className="mt-3 flex h-9 w-full items-center justify-center rounded-[1rem] border border-[var(--color-outline)] bg-[var(--color-surface-muted)] text-[var(--color-text-muted)] transition-colors hover:text-[var(--color-text)]"
          >
            <svg
              viewBox="0 0 24 24"
